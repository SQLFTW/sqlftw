...
class TimeInterval implements \SqlFtw\Sql\SqlSerializable
{
    ...
    /** @var \SqlFtw\Sql\Time\TimeIntervalUnit */
    private $unit;

    /**
     * @param int|string $value
     * @param \SqlFtw\Sql\Time\TimeIntervalUnit $unit
     */
    ...
    public static function createIntervals($interval): array
    {
        ...
        if ($interval->getDays() !== 0) {
            ...
            if ($interval->getMicroseconds() !== 0) {
                ...
                $intervals[] = new self(
                    sprintf('%d:%d:%d:%d', $interval->getHours(), $interval->getMinutes(), $interval->getSecondsFloored()),
                    TimeIntervalUnit::get(TimeIntervalUnit::HOUR_SECOND)
                );
            } elseif ($interval->getMinutes() !== 0) {
                $intervals[] = new self(
                    sprintf('%d:%d:%d:%d', $interval->getHours(), $interval->getMinutes()),
